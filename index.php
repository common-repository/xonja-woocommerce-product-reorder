...
include 'classes/Class_Xonja_Reorder_Handler.php';
...
function xonja_wc_reorder_do_page() {
    ...
    <div class="wrap">
        <div class="fixed-holder">
            ...
            <a class="button button-primary" href="<?php echo get_post(woocommerce_get_page_id('shop'))->guid; ?>" target="_blank"><?php echo __('Frontend Shop', 'xonja') ?></a>
            <button id="save_order" class="button button-primary"><?php echo __('Save order', 'xonja') ?></button>
        </div>
        <div id="inner">
            <?php
            $loop = Xonja_Reorder_Handler::init()->get_products_loop();
            if ($loop->have_posts()) {
...
function xonja_wc_ajax_reorder() {
    ...
    if (defined('DOING_AJAX')) {
        ...
        try {
            ...
            echo json_encode(['msg' => __('There was an error saving the sorting order', 'xonja') . ': ' . $e->getMessage(), 'error' => true]);
            die();
        }
        echo json_encode(['msg' => __('The sorting order has been saved at', 'xonja') . ': ' . date('H:i:s'), 'error' => false]);
        do_action('xonja_wc_after_page_reorder');
    }
    die();
}

add_action('wp_ajax_nopriv_xonja_wc_reorder', 'xonja_wc_ajax_reorder');
add_action('wp_ajax_xonja_wc_reorder', 'xonja_wc_ajax_reorder');
